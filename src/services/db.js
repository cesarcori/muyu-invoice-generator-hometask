...
async function initDB() {
  const pk = isTest ? 'INTEGER PRIMARY KEY AUTOINCREMENT' : 'SERIAL PRIMARY KEY';
  ...
  if (!isTest) {
    await query(`ALTER TABLE invoices ADD COLUMN IF NOT EXISTS owner_email TEXT;`);
  }
  await query(`CREATE INDEX IF NOT EXISTS idx_invoices_owner ON invoices(owner_email);`);
...
async function saveInvoice(invoiceData) {
  ...
  const result = await query(
    ...
    [companyName, companyDetails, taxRate, subtotal, total, isTest ? JSON.stringify(items) : items, owner_email]
  );
...
async function upsertProfile(profileData) {
  const { email, company_name, company_details, default_tax_rate } = profileData;
